...
class RolePage extends Component {
  ...
  handleModalSubmit = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (!id) {
        axios.post("/roles", fieldsValue, { headers }).then(() => {
          form.resetFields();
          this.setState({ modalVisible: false });
          this.handleRefresh();
        });
      } else {
        axios.put("/roles/" + id, fieldsValue, { headers }).then(() => {
          form.resetFields();
          this.setState({ modalVisible: false });
          this.handleRefresh();
        });
  ...
  handleDelete = id => {
    axios.delete("/roles/" + id, { headers }).then(() => {
      this.handleRefresh();
    });
  };
  ...
  render() {
    ...
    const getModalContent = () => {
      return (
        <Form>
          <FormItem label="Role name" hasFeedback {...this.formLayout}>
